...
func TestReadCoap_simplest(t *testing.T) {
	...
	assert(t, NewCoapPacket(CODE_205, []byte{}, []byte{}), coap)
}
...
func TestReadCoap_withToken(t *testing.T) {
	...
	assert(t, NewCoapPacket(CODE_203, []byte{0x7f}, []byte{}), coap)
}
...
func TestReadCoap_withPayload(t *testing.T) {
	...
	assert(t, NewCoapPacket(CODE_201, []byte{}, []byte{0x01, 0x02}), coap)
...
func TestReadCoap_withLargerPayload(t *testing.T) {
	...
	assert(t, NewCoapPacket(CODE_201, []byte{}, []byte{0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f}), coap)
...
func TestReadCoap_withUriPath(t *testing.T) {
	...
	expectedPacket := NewCoapPacket(CODE_203, []byte{}, []byte{})
...
func TestReadCoap_withContentFormat(t *testing.T) {
	...
	expectedPacket := NewCoapPacket(CODE_203, []byte{}, []byte{})
	expectedPacket.ContentFormat = CT_APPLICATION_OCTET_STREAM
	assert(t, expectedPacket, coap)
}

func TestWriteCoap_simplest(t *testing.T) {

	coap := NewCoapPacket(CODE_205, []byte{}, []byte{})
	w := new(bytes.Buffer)
...
func TestWriteCoap_token_and_payload(t *testing.T) {
	...
	coap := NewCoapPacket(CODE_205, []byte{0x01, 0x02}, []byte{0x10, 0x11, 0x12})
	w := new(bytes.Buffer)
...
func TestReadWriteCoap(t *testing.T) {
	...
	//token and payload
	coap2 := NewCoapPacket(CODE_205, []byte{0x01, 0x02}, []byte("test"))
	assert(t, coap2, writeAndRead(coap2, t))

	//all options
	coap2.ContentFormat = CT_APPLICATION_OCTET_STREAM
	coap2.MaxAge = 100
